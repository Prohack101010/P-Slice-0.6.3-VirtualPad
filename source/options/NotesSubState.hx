		...
		if(changingNote) {
			if(holdTime < 0.5) {
				if(controls.UI_LEFT_P) {
					...
					updateValue(1);
					FlxG.sound.play(Paths.sound('scrollMenu'));
				} else if(controls.RESET #if TOUCH_CONTROLS_ALLOWED || _virtualpad.buttonC.justPressed #end) {
			...
			if(controls.RESET #if TOUCH_CONTROLS_ALLOWED || _virtualpad.buttonC.justPressed #end) {
				for (i in 0...3) {
