...
class ModsMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		{
			trace("Mod settings are not avaliable on 0.6.3, idiot!");
		}, 54, 54);
...
function parseModsList() {
	...
	override function update(elapsed:Float)
	{
		...
		{
			if(controllerMode) FlxG.mouse.visible = false;
			_lastControllerMode = controllerMode;
		}
	...
	function updateModDisplayData()
	{
		...
		var modName_width = 40*modName.letters.length; // Feel free to hate me for this, but at least it's consistent
		var newScale = Math.min(620 / (modName_width / 0.8), 0.8);
	...
	{
		saveTxt();
		FlxG.autoPause = true;//ClientPrefs.data.autoPause;
...
class ModItem extends FlxSpriteGroup
{
	...
	public function new(folder:String)
	{
		...
		if(FileSystem.exists(file))
		{
			icon.loadGraphic(bmp, true, 150, 150);//! removed caching here. lol
